...
def resample(leftvalley, rightvalley, form_index, formant_frame, shift_const):
	print('Start here')
	print(formant_frame)
	print(formant_frame[i+1])
	print(shift_const + form_index>formant_frame[i+1])
	resampledarray = []
	print(shift_const)
	print(form_index)
	print(formant_frame[i+1])
	#if shift_const + form_index>formant_frame[i+1]:
	#	shift_const =  formant_frame[i+1]-shift_const + form_index
	if shift_const>= 0:
		left_array = signal.resample(leftvalley, abs(len(leftvalley)+shift_const))
		#print(left_array)
		print(shift_const)
		print(len(rightvalley)-shift_const)
		print(len(rightvalley))
...
print(len(sp))
print(len(formants[0])) # :( this is 158 not 159
print(formants[0][5])
...
#3.
#formants_matrix =maximas(sp, formants_matrix, frequency_axis)
#print(formants_matrix)

#4. 
maximas = func4(sp)
print(maximas[158])

#shiftspectrogram(sp, formants_matrix, maximas,  2, shift_array)
#print(frequency_axis)




print(formants_matrix[150])

#5.
norm_formant_matrix = normalize_maximas(maximas, formants_matrix, frequency_scale)
print(norm_formant_matrix[89])
print(maximas[158][0])



#shiftspectrogram(sp, norm_formant_matrix, maximas,  2, shift_array)
print('Starting here')
formants_matrix = norm_formant_matrix
shiftedspectrogram = []
for i in np.arange(0,len(sp)):
	spec_env = sp[i,:]
	print('it print all maximas')
	#print(maximas)
	#maximas = maximas[i]
	#print(maximas)
	#maximas = maximas[0]
	print(i)
	formant_frame = formants_matrix[i]
	print(maximas)
	#maximas = maximas[0]
	print(maximas)
	print('W2here is this')
	multi_shiftedarrays = []
	for j in np.arange(0,nos_peaks):
		print(formants_matrix[i])
		print(formant_frame[i])
		leftvalley,rightvalley, leftvalue, rightvalue = ppparray(spec_env, formant_frame[j], maximas)
		resampledarray = resample(leftvalley, rightvalley, formant_frame[j], formant_frame, shift_array[j])
		shifted_specenv = zeroresampledarray(spec_env, leftvalue, rightvalue, shift_array[j], resampledarray)
		multi_shiftedarrays.append(shifted_specenv[0])
	print(len(multi_shiftedarrays))
	print(multi_shiftedarrays)
	shiftedspecframe = np.max(multi_shiftedarrays,axis=0)
	print('finish max spec frame')
	for g in np.where(shiftedspecframe == 0)[0]:
