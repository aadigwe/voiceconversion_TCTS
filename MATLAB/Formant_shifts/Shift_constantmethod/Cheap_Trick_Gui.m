function spectrum_parameter = Cheap_Trick_Gui(x, fs, source_object, start_time, option)
...
spectrogram = zeros(fft_size / 2 + 1, length(f0_sequence));
        ...
        %if start_time == round(temporal_positions(i),3)%%%%%%%%%%%%MIN ABS FOR 0.005ROUNDED TEMPORAL POSITIONS TO TWO DECIMAL PLACES
           [~,index] = min(abs(temporal_positions-start_time));

 
            disp(index)
            disp(temporal_positions(index))
            disp(f0_sequence(index))
            waveform= GetWindowedWaveform(x, fs, f0_sequence(index), temporal_positions(index));
            power_spectrum = GetPowerSpectrum(waveform, fs, fft_size, f0_sequence(index));
            smoothed_spectrum = LinearSmoothing(power_spectrum, f0_sequence(index), fs, fft_size);
            spectral_envelope = SmoothingWithRecovery(...
                  [smoothed_spectrum; smoothed_spectrum(end - 1 : -1 : 2)], f0_sequence(index), fs,...
                  fft_size, q1);

% output parameters
spectrum_parameter.waveform=waveform;
spectrum_parameter.power_spectrum = power_spectrum;
spectrum_parameter.smoothed_spectrum = smoothed_spectrum;
spectrum_parameter.spectral_envelope = spectral_envelope;
spectrum_parameter.temporal_positions = temporal_positions;
spectrum_parameter.fs = fs;
spectrum_parameter.fft_size=fft_size;
